...
async function writeWeather(ev) {
  const currentTempDisplay = document.querySelector(
    ".current-temp .temperature",
  );
  const currentDateDisplay = document.querySelector(".current-temp .date");

  const averageTempStatDisplay = document.querySelector(
    ".forecast-container .average-temp .stat",
  );
  const averagePrecipStatDisplay = document.querySelector(
    ".forecast-container .average-precipitation .stat",
  );
  const maxTempStatDisplay = document.querySelector(
    ".forecast-container .max-temp .stat",
  );

  const response = await fetch(
    "https://api.open-meteo.com/v1/forecast?latitude=-33.8974&longitude=150.9345&hourly=temperature_2m,rain&current=temperature_2m,rain&timezone=auto&forecast_days=1",
  );
  ...
  try {
    ...
    const averageTemp =
      hourlyTemps.reduce((previous, current) => previous + current) /
      hourlyTemps.length;
    ...
    const averagePrecip =
      hourlyPrecips.reduce((previous, current) => previous + current) /
      hourlyPrecips.length;
    averagePrecipStatDisplay.textContent =
      averagePrecip.toFixed(1) + precipUnit;
  } catch (error) {
    console.log(error);
  }
}   
...
function setVisibleForStatDisplayLabels(bool) {
  const statDisplayLabels = document.querySelectorAll(
    ".forecast-container .stat-box .label",
  );
